...
import CategoryPoolButtons from '../misiki/CategoryPoolButtons.svelte'
import CheckboxOfMultiProducts from '$lib/ui/CheckboxOfMultiProducts.svelte'
import Cookie from 'cookie-universal'
import dayjs from 'dayjs'
import DeliveryOptions from '../misiki/DeliveryOptions.svelte'
import FrequentlyBoughtProduct from '../misiki/FrequentlyBoughtProduct.svelte'
import Gallery from '$lib/components/Product/Gallery.svelte'
import LazyImg from '$lib/components/Image/LazyImg.svelte'
import PrimaryButton from '$lib/ui/PrimaryButton.svelte'
import ProductAttributes from '../misiki/ProductAttributes.svelte'
...
<div class="min-h-screen lg:p-10">
	<div class="md:container md:mx-auto flex flex-col gap-5">
        ...
        {#if !data.product?.isCustomized}
          <div
            ...
            {#if currentVariantPrice >= 0} <!-- always show-->
              <div class="col-span-1">
        ...
        <div>
              ...
              {/if}
				...
				<!-- Specifications except text-table and markdown -->

				{#if data.product?.specifications?.filter(s=> s?.type !== 'text-table' && s?.type !== 'markdown')?.length}
					<div>
						<div class="mb-2 flex items-center gap-2 uppercase">
    ...
    <!-- Specifications only text-table and markdown -->
    {#if data.product?.specifications?.filter(s=> s?.type === 'text-table' || s?.type === 'markdown')?.length}
      <hr />
      <div>
        <div class="mb-2 flex items-center gap-2 uppercase">
        ...
        <div class="m-0 p-1 list-none flex flex-col gap-1">
          {#each data.product?.specifications?.filter(s=> s?.type === 'text-table' || s?.type === 'markdown') as s, index}
            {#if (index === 0)}
              <div class="flex flex-row gap-1 p-1">
                <div class="flex-1">
